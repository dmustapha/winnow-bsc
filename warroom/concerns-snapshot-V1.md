...
## PER-HACKATHON (Build the Era, added 2026-09-08 [AUTO])
- All 4 categories equal depth — single-category submissions score poorly (verbatim from rubric)
- Agents surfaced must be LIVE on BSC (eligibility gate)
- Deployment must stay publicly accessible Sep 9–23 (judging window)
- TermiX Agent Advantage Report planned from day one (30% of TermiX score)
- No video/live-URL fields in the form — GitHub README carries all proof links
- Hard wall-clock: submission by 2026-09-09 12:00 UTC (~13h) — scope must be shippable
- Forbidden-shapes list (#15): AgentMesh (registry+mesh+micropay), AlphaAttest (commit-resolve accountability core); Agent Auditor Sybil kernel = combinable [HISTORY] only
