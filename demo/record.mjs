...
const results = { scenes: [], fallbacks: [], mySessionId: null, startedAt: new Date().toISOString() };
...
async function main() {
  ...
  // ---------- Scene 1: Landing (20s) ----------
  await scene("scene1-landing", async (page) => {
    await gotoAndDress(page, BASE + "/");
    await cap(page, "Two hundred thousand agents registered on BSC under ERC-8004.");
    await sleep(1500);
    const tiles = page.locator('a[href^="/c/"]');
    const n = Math.min(await tiles.count(), 4);
    for (let i = 0; i < n; i++) {
      const b = await tiles.nth(i).boundingBox();
      if (b) await glide(page, b.x + b.width / 2, b.y + b.height / 2, 22);
      await sleep(1100);
      if (i === 1) await cap(page, "Most never answer a probe. Winnow finds the ones that do.");
    }
    await cap(page, "Every number here is recomputed from live probes, not vibes.");
    await glide(page, 960, 300, 25);
    await sleep(4000);
    await cap(page, "");
  });

  // ---------- Scene 2: Category browse (15s) ----------
  await scene("scene2-category", async (page) => {
    await gotoAndDress(page, BASE + "/");
    await cap(page, "Pick a job. Health factor monitoring.");
    const tile = page.locator('a[href="/c/health-factor"]').first();
    ...
    await cap(page, "Pick a job. Health factor monitoring.");
    await sleep(1200);
    await smoothScroll(page, 250, 1400);
    await cap(page, "Each agent carries a grade you can re-derive yourself.");
    await sleep(1800);
    const sentinel = page.locator('a:has-text("Winnow Sentinel")').first();
    const sb = await sentinel.boundingBox();
    if (sb) await glide(page, sb.x + 60, sb.y + sb.height / 2, 20);
    await sleep(900);
  ...
  // ---------- Scene 3: Re-probe live (30s) ----------
  await scene("scene3-reprobe", async (page) => {
    await gotoAndDress(page, AGENT);
    await cap(page, "Here is the grade broken down. Liveness, metadata, feedback, track record.");
    // hover the 4 meters
    const meters = page.locator('[role="meter"]');
    const mCount = Math.min(await meters.count(), 4);
    for (let i = 0; i < mCount; i++) {
      const b = await meters.nth(i).boundingBox();
      if (b) await glide(page, b.x + b.width * 0.6, b.y, 14);
      await sleep(700);
    }
    // open transcript drawer briefly
    const drawer = page.locator('summary:has-text("Raw probe transcript")').first();
    if (await drawer.count()) {
      await drawer.click();
      await sleep(2200);
      await drawer.click();
    }
    await cap(page, "Do not trust it. Re-probe it.");
    await sleep(1200);
    const btn = page.getByRole("button", { name: /Re-probe now|Probe now/ }).first();
    if (await btn.count()) {
      const b = await btn.boundingBox();
      if (b) await glide(page, b.x + b.width / 2, b.y + b.height / 2, 16);
      await btn.click();
      await cap(page, "One click, a real probe hits the agent endpoint, and the grade recomputes live.");
      await page.getByRole("button", { name: /Re-probe in \d+s/ }).first()
        .waitFor({ timeout: 12000 }).catch(() => {});
      await sleep(2500);
      // hover the fresh timestamp
      const ts = page.locator('h2:has-text("Grade breakdown")').first();
      const tb = await ts.boundingBox();
      if (tb) await glide(page, tb.x + 320, tb.y + 10, 16);
      await sleep(3000);
    } else {
      // cooldown fallback: point at the fresh timestamp
      results.fallbacks.push("scene3: reprobe on cooldown, showed fresh timestamp instead");
      await cap(page, "Probed seconds ago. The timestamp is the receipt.");
      await sleep(5000);
    }
    await cap(page, "");
  });
  ...
  fs.writeFileSync(".scenes.json", JSON.stringify(results, null, 2));
