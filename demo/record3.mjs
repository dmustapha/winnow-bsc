...
async function main() {
  ...
  results.mySessionId = 9;
  results.note = "session #9 granted via rehearsed curl (the budgeted activate); scene 4 uses the script fallback beat on the fresh row";
  const my = 9;

  // ---------- Scene 4 (fallback beat): the freshly granted session row ----------
  await scene("scene4-session-row", async (page) => {
    ...
    await cap(page, "The Altana relay granted a session key on chain, moments ago.");
    const rb = await row.boundingBox().catch(() => null);
    if (rb) await glide(page, rb.x + 180, rb.y + 24, 22);
    await sleep(3500);
    await cap(page, "A hard spend cap and an expiry, written into the Keystore.");
    if (rb) await glide(page, rb.x + 320, rb.y + 60, 20);
    await sleep(3500);
    await cap(page, "There it is. Capped at five thousandths of a BNB, expiring in 24 hours.");
    if (rb) await glide(page, rb.x + 500, rb.y + 30, 22);
    await sleep(4500);
    await cap(page, "");
  });
  ...
  // safety verification: sessions 5 and 6 untouched
  const post = await api("/api/sessions");
  const s5s = post.items.find((s) => s.id === 5), s6s = post.items.find((s) => s.id === 6);
  results.safety = { s5_status: s5s?.status, s6_status: s6s?.status };
  results.finishedAt = new Date().toISOString();
  fs.writeFileSync(".scenes-take3.json", JSON.stringify(results, null, 2));
  console.log("SAFETY session#5:", s5s?.status, "session#6:", s6s?.status);
