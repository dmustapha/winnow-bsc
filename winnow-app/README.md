...
Next.js 14 (App Router) · better-sqlite3 · viem · `@altananetwork/sdk` 0.9.0 · pluggable OpenAI-compatible LLM gateway for agent reasoning (AgentRouter / DeepSeek, host env configured, with Anthropic and local Claude CLI fallbacks) · Docker on Render. No custom contracts: Winnow composes the canonical ERC-8004 registries and the Altana Keystore, which is the point: anything it writes, every other agent app can read.
