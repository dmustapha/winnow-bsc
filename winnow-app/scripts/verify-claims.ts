...
const recompute = {
  ...
  // STRESS FIX (ST-API-4): a grade must belong to an indexed agent — catches ghost-agent grade rows
  gradesWithoutAgent: (db.prepare("SELECT COUNT(*) c FROM grades g LEFT JOIN agents a ON a.chain_id=g.chain_id AND a.token_id=g.token_id WHERE a.token_id IS NULL").get() as any).c,
...
if (recompute.orphanGrades > 0 || recompute.gradesWithoutAgent > 0 || recompute.negativeAttestations > 0) { console.error("INVARIANT VIOLATION"); process.exit(1); }
