...
## Simplified
- **Writes are BSC testnet (chainId 97); market-data reads are BSC mainnet (chainId 56).** Every explorer link is labeled with its chain. The Altana track explicitly accepts testnet. The mainnet migration path (re-run registration, attestation, and one action per agent on chain 56, about $6 of gas) uses the identical code already proven on testnet.
...
- **Mainnet deployment** is pending funding (about 0.003 to 0.01 BNB on chain 56); the runbook is documented and the code is proven on testnet.
